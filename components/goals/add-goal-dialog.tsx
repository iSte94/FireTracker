...
import type { GoalFormData, AssetClass } from '@/types/investment';
...
export function AddGoalDialog({ open, onOpenChange, onGoalAdded }: AddGoalDialogProps) {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      title: '',
      description: '',
      goal_type: 'target_portfolio_value',
      target_value: 0,
    },
  });
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      await goalsClient.createGoal(goalData);
      onGoalAdded();
      onOpenChange(false);
      form.reset();
      setAllocations([]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Definisci un nuovo obiettivo di investimento per monitorare i tuoi progressi finanziari
          </DialogDescription>
        </DialogHeader>
